...
def init_db():
    ddl_statements = [
        """CREATE TABLE IF NOT EXISTS questions (
            ...
            option_a       VARCHAR(4000) NOT NULL,
            option_b       VARCHAR(4000) NOT NULL,
            option_c       VARCHAR(4000) NOT NULL,
            option_d       VARCHAR(4000) NOT NULL,
            correct_option VARCHAR(10)   NOT NULL,
            explanation    TEXT,
            source_title   VARCHAR(4000),
        ...
        """CREATE TABLE IF NOT EXISTS tests (
            ...
            created_at         TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            user_id            INTEGER,
            competency_applied INTEGER DEFAULT 0
        )""",
        ...
        """CREATE TABLE IF NOT EXISTS users (
            id          INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            uid         VARCHAR(4000) UNIQUE NOT NULL,
        ...
        """CREATE TABLE IF NOT EXISTS user_skills (
            id               INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            user_id          INTEGER NOT NULL,
            skill_name       VARCHAR(4000) NOT NULL,
            self_rated_level VARCHAR(100),
        ...
        """CREATE TABLE IF NOT EXISTS skill_fallback_topics (
            skill_name   VARCHAR(4000) PRIMARY KEY,
            topics_json  TEXT,
            generated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )""",

        """CREATE TABLE IF NOT EXISTS user_staged_files (
            id         INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            user_uid   VARCHAR(4000) NOT NULL,
        ...
        """CREATE TABLE IF NOT EXISTS user_vector_chunks (
            user_uid    VARCHAR(4000) PRIMARY KEY,
            chunks_clob TEXT NOT NULL,
            updated_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )""",
        ...
        """CREATE TABLE IF NOT EXISTS learning_time (
            ...
            updated_at    TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT uq_learning_time UNIQUE (user_id, activity_date),
            CONSTRAINT fk_lt_user FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE
        )""",

        """CREATE TABLE IF NOT EXISTS nssta_calendar_events (
            id                    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            category              VARCHAR(300) NOT NULL,
            module_topic          VARCHAR(2000) NOT NULL,
            level_of_participants VARCHAR(500),
            start_date            DATE,
            end_date              DATE,
            dates_raw             VARCHAR(500),
            duration_text         VARCHAR(200),
            venue                 VARCHAR(1000),
            fiscal_year           VARCHAR(20) DEFAULT 'FY 2026-27',
            created_at            TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT uq_nssta_event UNIQUE (category, module_topic, dates_raw)
        )""",
    ]
...
def _update_skill_competency_and_progress(user_id, test_id):
    ...
    cur.execute("SELECT competency_applied FROM tests WHERE id = %s", (test_id,))
    already_applied = _fetchone(cur)
    if not already_applied or already_applied["competency_applied"]:
        conn.close()
        return

    cur.execute("SELECT skill_name FROM user_skills WHERE user_id = %s", (user_id,))
    ...
    if not declared_skills:
        cur.execute("UPDATE tests SET competency_applied = 1 WHERE id = %s", (test_id,))
        conn.commit()
        conn.close()
        return
        ...
        cur.execute(
            """INSERT INTO skill_competency (user_id, skill_name, correct_count, total_count)
               ...
               ON CONFLICT (user_id, skill_name) DO UPDATE SET
                   correct_count = skill_competency.correct_count + EXCLUDED.correct_count,
                   total_count   = skill_competency.total_count + 1,
    ...
    if matched_any:
        cur.execute(
            "SELECT correct_count, total_count FROM skill_competency WHERE user_id = %s", (user_id,)
        )
        ...
        cur.execute(
            "INSERT INTO progress_snapshots (user_id, test_id, overall_percent) VALUES (%s, %s, %s)",
...
LEARNING_HEARTBEAT_SECONDS = 30
LEARNING_HEARTBEAT_MAX_SECONDS = 60


def get_total_learning_seconds(user_id):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT COALESCE(SUM(seconds_spent), 0) AS total FROM learning_time WHERE user_id = %s", (user_id,))
    row = _fetchone(cur)
    conn.close()
...
def api_heartbeat():
    ...
    try:
        seconds = int(data.get("seconds", LEARNING_HEARTBEAT_SECONDS))
    except (TypeError, ValueError):
        seconds = LEARNING_HEARTBEAT_SECONDS
    seconds = max(1, min(seconds, LEARNING_HEARTBEAT_MAX_SECONDS))

    user = _get_user_row(request.uid)
    if not user:
    ...
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO learning_time (user_id, activity_date, seconds_spent)
           VALUES (%s, CURRENT_DATE, %s)
           ON CONFLICT (user_id, activity_date) DO UPDATE SET
               seconds_spent = learning_time.seconds_spent + EXCLUDED.seconds_spent,
               updated_at    = CURRENT_TIMESTAMP""",
        (user["id"], seconds)
    )
    conn.commit()
...
@app.route('/api/learning_hours/<uid>', methods=['GET'])
def api_learning_hours(uid):
    user = _get_user_row(uid)
    if not user:
        return jsonify({"error": "Profile not found."}), 404

    total_seconds = get_total_learning_seconds(user["id"])
    return jsonify({"total_seconds": total_seconds, "total_hours": seconds_to_hours(total_seconds)})
...
def get_staged_files_meta(uid):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, filename, source, title FROM user_staged_files WHERE user_uid = %(p_uid)s ORDER BY created_at",
        {"p_uid": uid}
    )
    rows = _fetchall(cur)
    conn.close()
...
def add_staged_file_meta(uid, filename, source, title, data_bytes):
    ...
    cur.execute(
        """INSERT INTO user_staged_files (user_uid, filename, source, title, file_blob)
           VALUES (%(p_uid)s, %(filename)s, %(source)s, %(title)s, %(file_blob)s)""",
...
def clear_staged_files_meta(uid):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("DELETE FROM user_staged_files WHERE user_uid = %(p_uid)s", {"p_uid": uid})
    cur.execute("DELETE FROM user_vector_chunks WHERE user_uid = %(p_uid)s", {"p_uid": uid})
    conn.commit()
    conn.close()
...
def remove_staged_file_meta(uid, filename):
    ...
    cur = conn.cursor()
    cur.execute(
        "DELETE FROM user_staged_files WHERE user_uid = %(p_uid)s AND filename = %(filename)s",
...
def get_staged_file_blobs(uid):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT filename, file_blob FROM user_staged_files WHERE user_uid = %(p_uid)s ORDER BY created_at",
        {"p_uid": uid}
    )
    rows = _fetchall(cur)
    conn.close()
    return [(r["filename"], bytes(r["file_blob"])) for r in rows]
...
def get_user_vector_store(uid):
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT chunks_clob FROM user_vector_chunks WHERE user_uid = %(p_uid)s", {"p_uid": uid})
    row = _fetchone(cur)
    conn.close()
...
def set_user_vector_store(uid, store, chunks):
    ...
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO user_vector_chunks (user_uid, chunks_clob)
           VALUES (%(p_uid)s, %(chunks_clob)s)
           ON CONFLICT (user_uid) DO UPDATE SET
               chunks_clob = EXCLUDED.chunks_clob,
               updated_at  = CURRENT_TIMESTAMP""",
...
def _get_cached_lab_content(topic, category):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT theory_clob, examples_clob, task_json FROM lab_content WHERE topic = %s AND category = %s",
        (topic, category)
    )
    row = _fetchone(cur)
    conn.close()
...
def _cache_lab_content(topic, category, theory, examples, task):
    ...
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO lab_content (topic, category, theory_clob, examples_clob, task_json)
           VALUES (%(p_topic)s, %(p_category)s, %(p_theory)s, %(p_examples)s, %(p_task)s)
           ON CONFLICT (topic, category) DO UPDATE SET
               theory_clob   = EXCLUDED.theory_clob,
               examples_clob = EXCLUDED.examples_clob,
...
def save_profile():
    ...
    if existing:
        user_id = existing["id"]
        cur.execute(
            """UPDATE users SET name=%s, age=%s, phone=%s, email=%s, department=%s, designation=%s
               WHERE id=%s""",
            (name, age, phone, email, department, designation, user_id)
        )
        cur.execute("DELETE FROM user_education WHERE user_id = %s", (user_id,))
        cur.execute("DELETE FROM user_skills WHERE user_id = %s", (user_id,))
        cur.execute("DELETE FROM user_experience WHERE user_id = %s", (user_id,))
    else:
        cur.execute(
    ...
    for edu in education:
        if not (edu.get("degree") or edu.get("institution")):
            continue
        cur.execute(
            "INSERT INTO user_education (user_id, degree, institution, year) VALUES (%s, %s, %s, %s)",
    ...
    for skill in skills:
        if not skill.get("name"):
            continue
        cur.execute(
            "INSERT INTO user_skills (user_id, skill_name, self_rated_level) VALUES (%s, %s, %s)",
...
def get_profile(uid):
    ...
    cur.execute(
        "SELECT degree, institution, year FROM user_education WHERE user_id = %s", (user["id"],)
    )
    education = _fetchall(cur)
    cur.execute(
        "SELECT role, organization, duration, description FROM user_experience WHERE user_id = %s", (user["id"],)
    )
...
def add_profile_skill(uid):
    ...
    cur.execute(
        "INSERT INTO user_skills (user_id, skill_name, self_rated_level) VALUES (%s, %s, %s) RETURNING id",
        (user["id"], name, level)
    )
...
def processed_status():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*) AS count FROM user_vector_chunks WHERE user_uid = %(p_uid)s", {"p_uid": request.uid})
...
def generate_test():
    ...
    if uid:
        ...
        user = _fetchone(cur)
        if user:
            cur.execute("UPDATE tests SET user_id = %s WHERE id = %s", (user["id"], result["test_id"]))
            conn.commit()
        conn.close()
...
def submit_test():
    ...
    if uid:
        ...
        if user:
            user_id = user["id"]
            cur.execute("UPDATE tests SET user_id = %s WHERE id = %s", (user_id, test_id))
...
def generate_skill_test():
    ...
    cur.execute(
        "INSERT INTO tests (difficulty, num_questions, source_title, user_id) VALUES (%s, %s, %s, %s) RETURNING id",
...
def get_my_tests(uid):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM tests WHERE user_id = %s ORDER BY created_at DESC", (user["id"],)
    )
    tests = _fetchall(cur)
    ...
    for t in tests:
        cur.execute(
            "SELECT * FROM test_attempts WHERE test_id = %s ORDER BY taken_at DESC LIMIT 1",
            (t["id"],)
        )
...
def get_skill_radar(uid):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT skill_name FROM user_skills WHERE user_id = %s", (user["id"],)
    )
    skill_rows = _fetchall(cur)
    declared_skills = [r["skill_name"] for r in skill_rows]

    cur.execute(
        "SELECT skill_name, correct_count, total_count FROM skill_competency WHERE user_id = %s",
        (user["id"],)
    )
    competency_rows = _fetchall(cur)
    conn.close()
...
def get_progress_history(uid):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT overall_percent, taken_at FROM progress_snapshots WHERE user_id = %s ORDER BY taken_at ASC",
        (user["id"],)
    )
    rows = _fetchall(cur)
    conn.close()
...
def _get_cached_fallback_topics(skill_name):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT topics_json FROM skill_fallback_topics WHERE skill_name = %s", (skill_name,)
    )
    row = _fetchone(cur)
    conn.close()
...
def _cache_fallback_topics(skill_name, topics):
    ...
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO skill_fallback_topics (skill_name, topics_json)
           VALUES (%s, %s)
...
def admin_delete_user(user_id):
    ...
    cur.execute("DELETE FROM user_skills WHERE user_id = %s", (user_id,))
    cur.execute("DELETE FROM user_experience WHERE user_id = %s", (user_id,))
    cur.execute("DELETE FROM skill_reports WHERE user_id = %s", (user_id,))
...
def api_bootstrap_core():
    ...
    cur.execute("SELECT COUNT(*) AS count FROM user_vector_chunks WHERE user_uid = %(p_uid)s", {"p_uid": uid})
    processed = cur.fetchone()["count"] > 0
    ...
    if user:
        cur.execute("SELECT id, skill_name, self_rated_level FROM user_skills WHERE user_id = %s", (user["id"],))
        skills = _fetchall(cur)
        cur.execute("SELECT degree, institution, year FROM user_education WHERE user_id = %s", (user["id"],))
        education = _fetchall(cur)
        cur.execute("SELECT role, organization, duration, description FROM user_experience WHERE user_id = %s", (user["id"],))
        experience = _fetchall(cur)
        ...
        cur.execute("SELECT * FROM tests WHERE user_id = %s ORDER BY created_at DESC", (user["id"],))
        tests = _fetchall(cur)
        for t in tests:
            cur.execute(
                "SELECT * FROM test_attempts WHERE test_id = %s ORDER BY taken_at DESC LIMIT 1",
...
def get_labs(uid):
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT skill_name FROM user_skills WHERE user_id = %s", (user["id"],))
...
def api_nssta_calendar():
    ...
    cur = conn.cursor()
    cur.execute(
        """SELECT id, category, module_topic, level_of_participants, start_date, end_date,
                  duration_text, venue
           FROM nssta_calendar_events
    ...
    return jsonify({"year": year, "month": month, "events": [_serialize_nssta_event(r) for r in rows]})


@app.route('/api/nssta_calendar/unscheduled', methods=['GET'])
def api_nssta_calendar_unscheduled():
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
        """SELECT id, category, module_topic, level_of_participants, start_date, end_date,
           ...
           ORDER BY category, module_topic"""
    )
    rows = _fetchall(cur)
    conn.close()

    return jsonify({"events": [_serialize_nssta_event(r) for r in rows]})
...
def api_evaluate_lab():
    ...
    cur.execute(
        """INSERT INTO lab_attempts (user_id, topic, category, submission, score, feedback)
           VALUES (%(p_user_id)s, %(p_topic)s, %(p_category)s, %(p_submission)s, %(p_score)s, %(p_feedback)s)
           RETURNING id""",
        {
            "p_user_id": user_id, "p_topic": topic, "p_category": category,
            "p_submission": submission, "p_score": result["score"],
            "p_feedback": feedback_json_str
        }
    )
    cur.fetchone()
    conn.commit()
...
def trainer_api_login():
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM trainers WHERE username = %s", (username,))
...
def trainer_api_learners():
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM users WHERE department = %s", (department,))
    users = _fetchall(cur)
    ...
    if user_ids:
        cur.execute(
            """SELECT user_id, COALESCE(SUM(seconds_spent), 0) AS total_seconds
               FROM learning_time
               WHERE user_id = ANY(%s)
               GROUP BY user_id""",
            (user_ids,)
        )
        hours_map = {r["user_id"]: seconds_to_hours(r["total_seconds"]) for r in _fetchall(cur)}
    conn.close()
    ...
    for u in users:
        ...
        if report and report["overall_total"]:
            percent = round((report["overall_score"] / report["overall_total"]) * 100)
        learning_hours = hours_map.get(u["id"], 0)
        dept_total_hours += learning_hours
        result.append({
            "id": u["id"],
            "name": u["name"],
            "email": u["email"],
    ...
    return jsonify({
        ...
        "learners": result,
        "total_learning_hours": round(dept_total_hours, 1),
    })
...
def trainer_api_learner_detail(user_id):
    ...
    cur.execute("SELECT * FROM tests WHERE user_id = %s ORDER BY created_at DESC", (user_id,))
    tests = _fetchall(cur)
    ...
    total_learning_hours = seconds_to_hours(get_total_learning_seconds(user_id))
...
def trainer_api_assign_course():
    ...
    cur.execute(
        ...
        (user_id, session.get("trainer_id"), course_name, data.get("course_url", ""),
         data.get("organisation", ""), data.get("duration", ""), data.get("category", ""))
    )
    assigned_id = cur.fetchone()["id"]
    conn.commit()
    conn.close()
...
def trainer_api_learner_assigned_courses(user_id):
    ...
    cur.execute("SELECT * FROM assigned_courses WHERE user_id = %s ORDER BY assigned_at DESC", (user_id,))
    rows = _fetchall(cur)
    conn.close()
...
def trainer_api_remove_assigned_course(assignment_id):
    ...
    cur.execute(
        """SELECT ac.id FROM assigned_courses ac
           ...
           WHERE ac.id = %s AND u.department = %s""",
    ...
    cur.execute("DELETE FROM assigned_courses WHERE id = %s", (assignment_id,))
    conn.commit()
    conn.close()
...
def api_user_assigned_courses(uid):
    ...
    cur.execute("SELECT * FROM assigned_courses WHERE user_id = %s ORDER BY assigned_at DESC", (user["id"],))
    rows = _fetchall(cur)
    conn.close()
...
def _normalize_department(department):
    ...
    existing = [r["department"] for r in _fetchall(cur)]
    conn.close()
    for d in existing:
...
def admin_create_trainer():
    ...
    cur.execute("SELECT id FROM trainers WHERE LOWER(username) = LOWER(%s)", (username,))
    if _fetchone(cur):
        conn.close()
        return jsonify({"error": f"A trainer with username '{username}' already exists"}), 400

    cur.execute(
        "SELECT id FROM trainers WHERE LOWER(name) = LOWER(%s) AND LOWER(department) = LOWER(%s)",
        (name, department)
    )
    if _fetchone(cur):
        conn.close()
    ...
    cur.execute(
        ...
        (username, generate_password_hash(password), name, department)
    )
    trainer_id = cur.fetchone()["id"]
    conn.commit()
    conn.close()
...
def admin_delete_trainer(trainer_id):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT id FROM trainers WHERE id = %s", (trainer_id,))
...
def admin_reset_trainer_password(trainer_id):
    ...
    cur.execute("UPDATE trainers SET password_hash = %s WHERE id = %s", (generate_password_hash(password), trainer_id))
    conn.commit()
    conn.close()
...
def admin_department_audit():
    ...
    cur = conn.cursor()
    cur.execute("SELECT DISTINCT department FROM users WHERE department IS NOT NULL AND department != ' '")
    user_depts = [r["department"] for r in _fetchall(cur) if r["department"]]
    cur.execute("SELECT DISTINCT department FROM trainers")
    trainer_depts = [r["department"] for r in _fetchall(cur) if r["department"]]
    conn.close()
