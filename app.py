...
PREUPLOADED_BOOKS = [
    {
        "id": "python-book",
        "title": "Python Book",
        "author": " S.Venkata Lakshmi",
        "subject": "Python Programming",
        "filename": "basics_of_python_programming.pdf",
    },
    {
        ...
        "filename": "the_art_of_data_science.pdf",
    },
    {
        ...
        "filename": "project_management_skills.pdf",
    },
    {
    ...
    },
    {
        ...
        "author": "Srinath Beldona",
        "subject": "Cloud Computing",
        "filename": "introduction_to_cloud_computing.pdf",
    },
    {
        "id": "banking",
        ...
        "author": "BlitzLearn",
        "subject": "Banking",
        "filename": "fundamentals_of_banking.pdf",
    },
    {
        "id": "machine-learning",
        ...
        "filename": "programming_in_standard_ML.pdf",
    },
    {
        ...
        "filename": "introduction_to_artificial_intelligence.pdf",
    },
]
...
LAB_CATEGORY_KEYWORDS = {
    "code": [
        ...
        "programming", "coding", "software", "api", "django", "flask",
        "react", "node", "html", "css", "typescript", "git", "algorithm",
        "data structure",
    ],
    "cloud": [
        "cloud", "aws", "azure", "gcp", "devops", "kubernetes", "docker",
        "server", "infrastructure", "network", "cybersecurity", "security",
        "digital signature", "government cloud", "digital public infrastructure",
    ],
    "finance": [
        "finance", "financial", "accounting", "budget", "economics",
        "statistics", "statistical", "national accounts", "price statistics",
        "labour statistics", "agricultural statistics", "industrial statistics",
        "survey design", "sampling", "sdg indicators", "data quality",
        "metadata standards",
    ],
}

LAB_CATEGORY_GUIDES = {
    "code": "a hands-on programming exercise. The task should ask the learner to WRITE CODE that solves a well-defined problem related to the topic.",
    "cloud": "a hands-on cloud/infrastructure/digital-governance exercise. The task should ask the learner to describe/perform an external task (e.g. steps taken in a console, a config file, a command sequence) and explain or paste what they did, since they cannot run real cloud infrastructure inside this platform.",
    "finance": "a calculation-based exercise. The task should give concrete numeric data and ask the learner to perform a calculation, showing their working and final answer.",
    "general": "a hands-on applied exercise appropriate to the topic. The task should ask the learner to produce a written response demonstrating applied understanding.",
}
...
def init_db():
    ddl_statements = [
        ...
        """CREATE TABLE lab_content (
            ...
            generated_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT uq_lab_content UNIQUE (topic, category)
        )""",
        ...
        """CREATE TABLE trainers (
            id            NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            username      VARCHAR2(500) UNIQUE NOT NULL,
        ...
        """CREATE TABLE assigned_courses (
            ...
            user_id      NUMBER NOT NULL,
            trainer_id   NUMBER,
            course_name  VARCHAR2(2000) NOT NULL,
            course_url   VARCHAR2(2000),
            ...
            CONSTRAINT fk_ac_trainer FOREIGN KEY (trainer_id) REFERENCES trainers (id) ON DELETE SET NULL
        )""",
    ]
...
def _classify_lab_category(skill_name):
    lowered = (skill_name or "").lower()
    for category, keywords in LAB_CATEGORY_KEYWORDS.items():
        if any(kw in lowered for kw in keywords):
            return category
    return "general"

def _get_cached_lab_content(topic, category):
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
        "SELECT theory_clob, examples_clob, task_json FROM lab_content WHERE topic = :1 AND category = :2",
        (topic, category)
    )
    row = _fetchone(cur)
    conn.close()
    if not row or not row["theory_clob"]:
        return None
    return {
        "theory": row["theory_clob"],
        "examples": json.loads(row["examples_clob"]) if row["examples_clob"] else [],
        "task": json.loads(row["task_json"]) if row["task_json"] else {},
    }

    
def _cache_lab_content(topic, category, theory, examples, task):
    task_json_str = json.dumps(task)
    if len(task_json_str) > 4000:
        task = dict(task)
        task["prompt"] = (task.get("prompt") or "")[:1800]
        task["starter"] = (task.get("starter") or "")[:1500]
        task["success_criteria"] = (task.get("success_criteria") or "")[:400]
        task_json_str = json.dumps(task)[:4000]

    conn = get_db()
    cur = conn.cursor()
    cur.setinputsizes(
        p_topic=None, p_category=None,
        p_theory=oracledb.DB_TYPE_CLOB, p_examples=oracledb.DB_TYPE_CLOB
    )
    cur.execute(
        """MERGE INTO lab_content lc
           USING (SELECT :p_topic AS topic, :p_category AS category FROM DUAL) src
           ON (lc.topic = src.topic AND lc.category = src.category)
           WHEN MATCHED THEN UPDATE SET
               lc.theory_clob   = :p_theory,
               lc.examples_clob = :p_examples,
               lc.task_json     = :p_task,
               lc.generated_at  = CURRENT_TIMESTAMP
           WHEN NOT MATCHED THEN INSERT (topic, category, theory_clob, examples_clob, task_json)
               VALUES (:p_topic, :p_category, :p_theory, :p_examples, :p_task)""",
        {
            "p_topic": topic, "p_category": category, "p_theory": theory,
            "p_examples": json.dumps(examples), "p_task": task_json_str
        }
    )
    conn.commit()
    conn.close()
...
def _generate_lab_content(topic, category):
    guide = LAB_CATEGORY_GUIDES.get(category, LAB_CATEGORY_GUIDES["general"])

    prompt = f"""
    You are creating a self-contained hands-on lab for a working professional studying "{topic}"
    as part of India's Official Statistical System capacity-building programme.

    This lab should be {guide}

    Return ONLY a valid JSON object (no markdown fences, no commentary) with EXACTLY this shape:
    {{
      "theory": "A clear, well-structured explanation of the core concept(s) in {topic} needed to
        do the task below. Use plain paragraphs; you may use lines starting with '- ' for bullet
        points, and blank lines between paragraphs. 150-300 words.",
      "examples": [
        {{"title": "short example title", "content": "a concrete worked example illustrating the
          concept — for code topics, an actual short code snippet with real newlines; for other
          topics, a short worked walkthrough with real newlines between steps."}}
      ],
      "task": {{
        "prompt": "A specific, well-defined task instructing the learner to apply the concept.
          Include any numbers, sample data, or constraints needed to complete it fully
          self-contained — the learner will have no other materials.",
        "starter": "Starter content for the learner's workspace — for code: a minimal starter code
          template (or empty string); for other categories: an empty string or a short outline.",
        "success_criteria": "2-4 sentences describing what a correct, complete submission looks
          like — this will be used later to grade the learner's work."
      }}
    }}
    ...
    try:
        ...
        data = json.loads(_clean_json_response(response.content))
        theory = data.get("theory", "")
        examples = data.get("examples", [])
        task = data.get("task", {})
        if not theory or not task.get("prompt"):
            raise ValueError("Incomplete lab content generated")
        return theory, examples, task
    except Exception as e:
        print(f"Error generating lab content for {topic}/{category}: {str(e)}")
    ...
    try:
        ...
        cur.execute(
            """SELECT course_name, url, duration, organisation, category
               FROM igot_courses
               WHERE UPPER(course_name) LIKE UPPER(:pattern)
                  OR UPPER(description) LIKE UPPER(:pattern)
                  OR UPPER(category) LIKE UPPER(:pattern)
               ORDER BY course_name
               FETCH FIRST :max_rows ROWS ONLY""",
            {"pattern": pattern, "max_rows": limit}
        )
        rows = _fetchall(cur)
        conn.close()
        return rows
    ...
    try:
        ...
        cur.execute(
            """SELECT subject, reference_id, document_url, start_date, document_type
               FROM tpac_documents
               WHERE UPPER(subject) LIKE UPPER(:pattern)
...
@app.route('/api/labs/<uid>', methods=['GET'])
def get_labs(uid):
    user = _get_user_row(uid)
    if not user:
        return jsonify({"error": "Profile not found."}), 404
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT skill_name FROM user_skills WHERE user_id = :1", (user["id"],))
    declared_skills = [r["skill_name"] for r in _fetchall(cur)]
    ...
    if not competency_map:
        return jsonify({"has_data": False, "labs": []})
    ...
    for skill_name in declared_skills:
        ...
        labs.append({
            "topic": skill_name,
            "category": _classify_lab_category(skill_name),
            "current_percent": current_percent,
            "target_percent": target,
            "gap": gap,
            "priority": priority,
    ...
    labs.sort(key=lambda l: l["gap"], reverse=True)

    return jsonify({"has_data": True, "target_percent": target, "labs": labs})


@app.route('/labs-select')
def labs_select_page():
    return render_template('labs-select.html')
...
def api_lab_content():
    ...
    if category not in ("code", "cloud", "finance", "general"):
        category = "general"
    if not topic:
        return jsonify({"error": "Missing topic"}), 400

    cached = _get_cached_lab_content(topic, category)
    if cached:
        return jsonify({"topic": topic, "category": category, **cached})

    theory, examples, task = _generate_lab_content(topic, category)
    if theory is None:
        return jsonify({"error": "Failed to generate this lab. Please try again."}), 500

    _cache_lab_content(topic, category, theory, examples, task)

    return jsonify({"topic": topic, "category": category, "theory": theory, "examples": examples, "task": task})


def _evaluate_lab_submission(topic, category, task, submission):
    prompt = f"""
    You are grading a learner's submission for a hands-on lab on "{topic}" (category: {category}).

    Task given to the learner:
    {task.get("prompt", "")}

    What a correct, complete submission looks like:
    {task.get("success_criteria", "")}

    Learner's submission:
    {submission.strip() if submission and submission.strip() else "(the learner submitted nothing)"}

    Grade this submission fairly and constructively. Return ONLY a valid JSON object (no markdown
    ...
    {{
      ...
      "suggestions": ["a follow-up tip 1", "suggestion 2"]
    }}

    If the submission is empty or clearly does not attempt the task, score it low (0-15) and say so
    plainly in "summary". Keep each list to at most 4 items and specific to what was actually
    submitted — never generic filler.
    """
    ...
    try:
        ...
        print(f"Error evaluating lab submission: {str(e)}")
        return None, ("Failed to evaluate your submission. Please try again.", 500)


def _apply_lab_result_to_competency(user_id, topic, score):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT skill_name FROM user_skills WHERE user_id = :1", (user_id,))
    declared = {r["skill_name"].strip().lower(): r["skill_name"] for r in _fetchall(cur)}

    topic_key = topic.strip().lower()
    if topic_key not in declared:
        conn.close()
        return

    skill_name = declared[topic_key]
    is_pass = 1 if score >= 60 else 0

    cur.execute(
        """MERGE INTO skill_competency sc
           USING (SELECT :1 AS user_id, :2 AS skill_name, :3 AS is_pass FROM DUAL) src
           ON (sc.user_id = src.user_id AND sc.skill_name = src.skill_name)
           WHEN MATCHED THEN UPDATE SET
               sc.correct_count = sc.correct_count + src.is_pass,
               sc.total_count   = sc.total_count + 1,
               sc.updated_at    = CURRENT_TIMESTAMP
           WHEN NOT MATCHED THEN INSERT (user_id, skill_name, correct_count, total_count)
               VALUES (src.user_id, src.skill_name, src.is_pass, 1)""",
        (user_id, skill_name, is_pass)
    )
    conn.commit()
    conn.close()


@app.route('/api/evaluate_lab', methods=['POST'])
def api_evaluate_lab():
    data = request.json or {}
    uid = data.get("uid")
    topic = (data.get("topic") or "").strip()
    category = (data.get("category") or "general").strip().lower()
    ...
    if len(feedback_json_str) > 4000:
        trimmed = dict(result)
        trimmed["mistakes"] = trimmed.get("mistakes", [])[:2]
        trimmed["improvements"] = trimmed.get("improvements", [])[:2]
        trimmed["suggestions"] = trimmed.get("suggestions", [])[:2]
        feedback_json_str = json.dumps(trimmed)[:4000]
    ...
    id_var = cur.var(oracledb.NUMBER)
    cur.setinputsizes(p_submission=oracledb.DB_TYPE_CLOB)
    cur.execute(
        """INSERT INTO lab_attempts (user_id, topic, category, submission, score, feedback)
           VALUES (:p_user_id, :p_topic, :p_category, :p_submission, :p_score, :p_feedback)
           RETURNING id INTO :p_id""",
        {
            "p_user_id": user_id, "p_topic": topic, "p_category": category,
            "p_submission": submission, "p_score": result["score"],
            "p_feedback": feedback_json_str, "p_id": id_var
        }
    )
    conn.commit()
    conn.close()
    ...
    if user_id:
        _apply_lab_result_to_competency(user_id, topic, result["score"])
...
def trainer_required(view_func):
    ...
    def wrapped(*args, **kwargs):
        if not session.get("trainer_id"):
            if request.path.startswith("/trainer/api/"):
                return jsonify({"error": "Unauthorized"}), 401
...
@app.route('/trainer/api/login', methods=['POST'])
def trainer_api_login():
    data = request.json or {}
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM trainers WHERE username = :1", (username,))
    trainer = _fetchone(cur)
    conn.close()

    if not trainer or not check_password_hash(trainer["password_hash"], password):
        return jsonify({"error": "Invalid username or password"}), 401

    session["trainer_id"] = trainer["id"]
    session["trainer_name"] = trainer["name"]
    session["trainer_department"] = trainer["department"]

    return jsonify({"message": "Logged in", "name": trainer["name"], "department": trainer["department"]})


@app.route('/trainer/api/logout', methods=['POST'])
def trainer_api_logout():
    session.pop("trainer_id", None)
    session.pop("trainer_name", None)
    session.pop("trainer_department", None)
    return jsonify({"message": "Logged out"})
...
@app.route('/trainer/api/me', methods=['GET'])
@trainer_required
def trainer_api_me():
    return jsonify({"name": session.get("trainer_name"), "department": session.get("trainer_department")})


@app.route('/trainer/api/learners', methods=['GET'])
@trainer_required
def trainer_api_learners():
    department = session.get("trainer_department")

    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM users WHERE department = :1", (department,))
    users = _fetchall(cur)
    ...
    result.sort(key=lambda x: (x["name"] or "").lower())
    return jsonify({"department": department, "learners": result})


@app.route('/trainer/api/learner/<int:user_id>', methods=['GET'])
@trainer_required
def trainer_api_learner_detail(user_id):
    department = session.get("trainer_department")

    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM users WHERE id = :1", (user_id,))
    user = _fetchone(cur)

    if not user or (user["department"] or "") != department:
        conn.close()
        return jsonify({"error": "Learner not found in your department"}), 404
    ...
    cur.execute("SELECT * FROM tests WHERE user_id = :1 ORDER BY created_at DESC", (user_id,))
    tests = _fetchall(cur)
    ...
    competency = [
        {"skill": r["skill_name"], "percent": round((r["correct_count"] / r["total_count"]) * 100) if r["total_count"] else 0}
        for r in competency_rows
    ]
...
def _search_igot_courses(query, limit=20):
    try:
        import sqlite3
        ...
        if query:
            ...
            rows = conn.execute(
                """SELECT course_name, url, duration, organisation, category
                   FROM courses
                   WHERE course_name LIKE ? OR category LIKE ? OR organisation LIKE ?
                   ORDER BY course_name LIMIT ?""",
                (pattern, pattern, pattern, limit)
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT course_name, url, duration, organisation, category FROM courses ORDER BY course_name LIMIT ?",
                (limit,)
            ).fetchall()
        conn.close()
...
@trainer_required
def trainer_api_assign_course():
    data = request.json or {}
    user_id = data.get("user_id")
    course_name = (data.get("course_name") or "").strip()

    if not user_id or not course_name:
        return jsonify({"error": "Missing user_id or course_name"}), 400

    department = session.get("trainer_department")

    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT department FROM users WHERE id = :1", (user_id,))
    user = _fetchone(cur)
    if not user or (user["department"] or "") != department:
        conn.close()
        return jsonify({"error": "Learner not found in your department"}), 404
    ...
    id_var = cur.var(oracledb.NUMBER)
    cur.execute(
        """INSERT INTO assigned_courses (user_id, trainer_id, course_name, course_url, organisation, duration, category)
           VALUES (:1, :2, :3, :4, :5, :6, :7) RETURNING id INTO :8""",
        (user_id, session.get("trainer_id"), course_name, data.get("course_url", ""),
         data.get("organisation", ""), data.get("duration", ""), data.get("category", ""), id_var)
    )
    assigned_id = _getid(id_var)
    conn.commit()
    conn.close()

    return jsonify({"message": "Course assigned", "id": assigned_id})


@app.route('/trainer/api/learner/<int:user_id>/assigned_courses', methods=['GET'])
@trainer_required
def trainer_api_learner_assigned_courses(user_id):
    department = session.get("trainer_department")
    ...
    cur.execute("SELECT * FROM assigned_courses WHERE user_id = :1 ORDER BY assigned_at DESC", (user_id,))
    rows = _fetchall(cur)
    conn.close()
...
@app.route('/trainer/api/assigned_courses/<int:assignment_id>', methods=['DELETE'])
@trainer_required
def trainer_api_remove_assigned_course(assignment_id):
    department = session.get("trainer_department")

    conn = get_db()
    cur = conn.cursor()
    cur.execute(
        """SELECT ac.id FROM assigned_courses ac
           JOIN users u ON u.id = ac.user_id
           WHERE ac.id = :1 AND u.department = :2""",
        (assignment_id, department)
    )
    match = _fetchone(cur)
    if not match:
        conn.close()
    ...
    cur.execute("DELETE FROM assigned_courses WHERE id = :1", (assignment_id,))
    conn.commit()
    conn.close()
...
@app.route('/assigned-courses')
def assigned_courses_page():
    return render_template('assigned-courses.html')


@app.route('/api/assigned_courses/<uid>', methods=['GET'])
def api_user_assigned_courses(uid):
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT id FROM users WHERE "UID" = :1', (uid,))
    user = _fetchone(cur)
    ...
    cur.execute("SELECT * FROM assigned_courses WHERE user_id = :1 ORDER BY assigned_at DESC", (user["id"],))
    rows = _fetchall(cur)
    conn.close()
