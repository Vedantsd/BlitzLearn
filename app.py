...
from firebase_admin import storage as fb_storage_module
...
def init_db():
    """Creates all application tables in the Oracle schema if they do not
    already exist. Each CREATE TABLE is attempted individually; DDL failures
    (e.g. ORA-00955: name already used) are silently swallowed."""
    ddl_statements = [
...
fb_bucket = fb_storage_module.bucket()
...
def get_staged_files_meta(uid):
    snap = _user_doc_ref(uid).get()
    data = snap.to_dict() or {}
    return data.get("staged_files", [])


def add_staged_file_meta(uid, entry):
    files = get_staged_files_meta(uid)
    files.append(entry)
    _user_doc_ref(uid).set({"staged_files": files}, merge=True)


def clear_staged_files_meta(uid):
    _user_doc_ref(uid).set(
        {"staged_files": [], "processed": False, "chunks_path": None}, merge=True
    )


def remove_staged_file_meta(uid, filename):
    files = get_staged_files_meta(uid)
    remaining = [f for f in files if f["filename"] != filename]
    _user_doc_ref(uid).set({"staged_files": remaining}, merge=True)
    return len(files) - len(remaining)


def _note_storage_path(uid, filename):
    return f"notes/{uid}/{filename}"


def upload_note_bytes(uid, filename, data_bytes):
    path = _note_storage_path(uid, filename)
    blob = fb_bucket.blob(path)
    blob.upload_from_string(data_bytes, content_type="application/pdf")
    return path


def download_note_bytes(storage_path):
    blob = fb_bucket.blob(storage_path)
    return blob.download_as_bytes()


def _chunks_storage_path(uid):
    return f"chunks/{uid}.json"


def upload_chunks(uid, chunks):
    path = _chunks_storage_path(uid)
    blob = fb_bucket.blob(path)
    blob.upload_from_string(json.dumps(chunks), content_type="application/json")
    return path


def download_chunks(storage_path):
    blob = fb_bucket.blob(storage_path)
    if not blob.exists():
        return None
...
def get_user_vector_store(uid):
    ...
    chunks = download_chunks(chunks_path)
    if not chunks:
        return None
...
def set_user_vector_store(uid, store, chunks):
    _vector_store_cache[uid] = store
    chunks_path = upload_chunks(uid, chunks)
    _user_doc_ref(uid).set({"chunks_path": chunks_path, "processed": True}, merge=True)
...
def firebase_config_js():
    ...
    // Wraps fetch() so every call automatically carries the signed-in
    // user's Firebase ID token. The backend verifies this token to know
    // WHO is calling — required for any route that reads/writes a user's
    // own notes, settings, or vector store (see require_uid in app.py).
...
def upload_stage():
    ...
    for f in files:
        data_bytes = f.read()
        storage_path = upload_note_bytes(uid, f.filename, data_bytes)
        add_staged_file_meta(uid, {
            "filename": f.filename,
...
def select_book():
    ...
    storage_path = upload_note_bytes(uid, f"{book_id}-{book['filename']}", data_bytes)
    add_staged_file_meta(uid, {
...
def processed_status():
    snap = _user_doc_ref(request.uid).get()
    data = snap.to_dict() or {}
    return jsonify({"processed": bool(data.get("processed"))})
...
def process_content():
    ...
    raw_text = ""
    for f in staged:
        data_bytes = download_note_bytes(f["storage_path"])
...
def questions_bank():
    """Lists how many stored questions exist per topic/difficulty."""
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
...
def get_roadmap(uid):
    """Builds the personalised learning roadmap for a user:
    - Skill gap vs designation-adjusted target
    - iGOT course recommendations per skill
    - NSSTA/TPAC document recommendations per skill
    - AI-generated fallback sub-topics when no external resources are found
    Skills are sorted by gap descending (highest priority first)."""
    user = _get_user_row(uid)
    if not user:
        return jsonify({"error": "Profile not found."}), 404
